...
impl Document {
    fn h_serialize(&self, out: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        serialize_list(out, self.fields.iter(), |field, out| {
            ...
            field.value.h_serialize(out, definition);
            Ok(())
        });
        write!(out, "}}")?;
        Ok(())
...
impl FieldValue {
    fn h_serialize(&self, out: &mut impl Write, definition: &Field) -> Result<(), std::io::Error> {
        match self {
            ...
            FieldValue::Array(a) => {
                ...
                serialize_list(out, a.iter(), |value, out| {
                    value.h_serialize(out, definition)
                })?;
                write!(out, "]")?;
                Ok(())
            }
            FieldValue::Object(o) => o.h_serialize(out),
            FieldValue::Enum(e) => {
